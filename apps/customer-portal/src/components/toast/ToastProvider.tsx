...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const show = useCallback<ToastContextValue['show']>(
    ({ tone, title, message, durationMs = DEFAULT_DURATION_MS }) => {
      ...
      setToasts((prev) => {
        ...
        // Cap visible count so long-lived errors don't stack up.
  ...
  // The AuthProvider can't call useToast directly (it wraps this provider).
  // Instead it dispatches a window event on 401. We listen and surface a toast.
  useEffect(() => {
    const handler = () => {
      show({
        tone: 'warning',
...
function ToastItem({
  ...
  return (
    <div
      role="status"
      className={`pointer-events-auto relative overflow-hidden rounded-xl border ${style.border} bg-white shadow-lg shadow-gray-900/5 animate-toast-in-top`}
    >
      <span
        aria-hidden="true"
